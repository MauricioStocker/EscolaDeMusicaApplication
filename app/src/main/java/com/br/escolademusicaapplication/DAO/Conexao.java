...
// excluir em cataca um e após o outro
		...
		String sql_curso_escola = "CREATE TABLE curso (" +
				...
				"curso_imagem TEXT);";
		String sql_turma = "CREATE TABLE Turma (" +
	...
	public void salvarTurma(int professorId, List<Aluno_Professor> listaAlunosProfessor, String dataEncerramento) {
		if (db == null) {
			db = getWritableDatabase();
		}

		Gson gson = new Gson();
		String alunosJson = gson.toJson(listaAlunosProfessor);

		ContentValues values = new ContentValues();
		values.put("professor_id", professorId);
		values.put("alunos", alunosJson);
		values.put("data_encerramento", dataEncerramento);

		// Inserir a turma na tabela Turma
		long turmaId = db.insert("Turma", null, values);
		Log.d("TAG", "Turma salva com ID: " + turmaId);

		// Limpar o relacionamento aluno-professor para o professorId
		limparRelacionamentoAlunoProfessor(professorId);
		...
		try {
			if (cursor.moveToFirst()) {
				do {
					...
					turma.setTurma_id(cursor.getInt(cursor.getColumnIndexOrThrow("turma_id")));
					turma.setProfessor_id(cursor.getInt(cursor.getColumnIndexOrThrow("professor_id")));
					turma.setAlunos(cursor.getString(cursor.getColumnIndexOrThrow("alunos")));
					turma.setData_encerramento(cursor.getString(cursor.getColumnIndexOrThrow("data_encerramento")));

					Log.d("Conexao", "Turma ID: " + turma.getTurma_id());
					Log.d("Conexao", "Professor ID: " + turma.getProfessor_id());
					Log.d("Conexao", "Alunos: " + turma.getAlunos());
					Log.d("Conexao", "Data de Encerramento: " + turma.getData_encerramento());

					turmas.add(turma);
				} while (cursor.moveToNext());
			}
		} catch (Exception e) {
			Log.d("Conexao", "Erro ao buscar turmas: " + e.getMessage());
		} finally {
			cursor.close();
		}
	...
	public void limparRelacionamentoAlunoProfessor(int professorId) {
		...
		try {
			db.delete("Aluno_Professor", "id_professor = ?", new String[]{String.valueOf(professorId)});
			Log.d("TAG", "Relacionamento aluno-professor removido para o professor com ID " + professorId);
		} catch (Exception e) {
	...
	public String buscarNomeProfessor(int professorId) {
		...
		String query = "SELECT professor_nome FROM Professor WHERE professor_id = ?";
		Cursor cursor = db.rawQuery(query, new String[]{String.valueOf(professorId)});

		try {
			if (cursor.moveToFirst()) {
				nomeProfessor = cursor.getString(cursor.getColumnIndexOrThrow("professor_nome"));
			}
		} catch (Exception e) {
			Log.d("Conexao", "Erro ao buscar nome do professor: " + e.getMessage());
		} finally {
			cursor.close();
		}
